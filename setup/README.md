...
- SSH 接続を完全廃止、SSM のみ使用
- GPU 数の動的検出機能を追加
- Proxy Server のデプロイを追加
- タスク定義を JSON に統一
...
NODE1_INSTANCE_ID="i-xxxxx"           # Producer インスタンス ID
NODE2_INSTANCE_ID="i-xxxxx"           # Consumer インスタンス ID
...
1. NIXL fork のクローン（feature/complete-request-response-protocol ブランチ）
...
- **ブランチ**: `feature/complete-request-response-protocol`
- **コミット**: 190c74f

### Two-Sided RDMA Request/Response Protocol

AWS EFA は one-sided RDMA（fi_read/fi_write のみ）をサポートしていないため、two-sided messaging（fi_senddata + fi_recv）を使用した Request/Response protocol を実装。

**追加された実装: **

1. **ProducerTransferContext** 構造体
   - Producer 側の転送コンテキスト管理
   - フィールド: buffer, length, rail_mr_list, selected_rails, remote_agent, agent_index

2. **Control Rail メソッド**
   - `isControlRail()`: Rail 0 判定
   - `setControlMessageHandler()`: 制御メッセージハンドラ登録
   - `sendControlMessage()`: 制御メッセージ送信

3. **Rail Manager メソッド**
   - `getRailPtr()`: Control rail へのアクセス

4. **Producer-side tracking**
   - `producer_transfers_mutex_` (mutable)
   - `producer_transfers_` (mutable)

5. **定数追加**
   - `NIXL_LIBFABRIC_GIGA`: トポロジー計算用（10^9）
...
aws ssm send-command \
  --instance-ids i-027e2dc8698de14d6 \
...
```bash
aws ssm send-command \
  --instance-ids i-0a486b0b3395591a3 \
  --document-name AWS-RunShellScript \
