#!/bin/bash
##
## Complete NIXL Deployment Script (v6)
## Phase 3 complete pattern - includes kv-transfer-config and all NIXL parameters
##
## Usage:
##   ./deploy-v6.sh <config-file>
##
## Example:
##   ./deploy-v6.sh configs/phase3-us-west-2-v3.env
##
...
##   4. Setup Producer node with vLLM + NIXL + kv-transfer-config
##   5. Setup Consumer node with vLLM + NIXL + kv-transfer-config
##   6. Deploy custom LIBFABRIC plugin
##   7. Create startup scripts with proper NIXL environment variables
##
...
echo ""
echo "========================================="
echo "Deployment Configuration (v6)"
echo "========================================="
...
echo ""
echo "Common ENGINE_ID: ${ENGINE_ID:-nixl-efa-deployment}"
...
export ENGINE_ID="${ENGINE_ID:-nixl-efa-deployment-$(date +%Y%m%d)}"
...
TASK_FILE="$SCRIPT_DIR/tasks/complete-deployment-v6.json"
