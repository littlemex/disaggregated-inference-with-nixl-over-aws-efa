...
int runProducer(const Config &cfg) {
    ...
    // 5. Send metadata to Consumer (NIXL listener thread handles this)
    st = agent.sendLocalMD();
    nixl_exit_on_failure(st, "Failed to send local metadata", PRODUCER_NAME);
    std::cout << "[Producer] Metadata sent\n";

    // 6. Wait for Consumer metadata
    ...
    // 11. Wait for transfer completion notification (genNotif/getNotifs)
    std::cout << "[Producer] Waiting for transfer completion notification...\n";
    bool transferDone = false;
    startTime = std::chrono::steady_clock::now();

    while (!transferDone) {
        nixl_notifs_t notifs;
        st = agent.getNotifs(notifs);
        nixl_exit_on_failure(st, "Failed to get notifs", PRODUCER_NAME);

        if (notifs.count(CONSUMER_NAME) > 0) {
            for (const auto &msg : notifs[CONSUMER_NAME]) {
                std::cout << "[Producer] Received notification: " << msg << "\n";
                if (msg == "TRANSFER_DONE") {
                    transferDone = true;
                    break;
                }
            }
        ...
        auto elapsed = std::chrono::steady_clock::now() - startTime;
        if (std::chrono::duration_cast<std::chrono::seconds>(elapsed).count() > TIMEOUT_SEC) {
            std::cerr << "[Producer] Timeout waiting for TRANSFER_DONE notification\n";
            return 1;
        }
        if (!transferDone) {
...
int runConsumer(const Config &cfg) {
    ...
    std::cout << "[Consumer] Metadata sent to Producer\n";

    // 6. Fetch Producer metadata
    ...
    // 8. Deserialize Producer's descriptor list
    nixlSerDes remoteSerdes;
    remoteSerdes.importStr(remoteDescStr);
    nixl_xfer_dlist_t remoteDescs(&remoteSerdes);
    std::cout << "[Consumer] Remote descriptor list deserialized\n";

    nixl_xfer_dlist_t localDescs = regDescs.trim();

    // 9. Create and post READ transfer request
    std::cout << "\n[Consumer] === Initiating READ transfer ===\n";
    nixlXferReqH *xferReq = nullptr;
    nixl_opt_args_t xferParams;
    xferParams.backends.push_back(bknd);
    xferParams.notifMsg = "TRANSFER_DONE";  // Notification for completion only
    xferParams.hasNotif = true;

    int retryCount = 0;
    do {
        st = agent.createXferReq(NIXL_READ, localDescs, remoteDescs,
                                 PRODUCER_NAME, xferReq, &xferParams);
        if (st != NIXL_SUCCESS) {
            retryCount++;
            if (retryCount % 50 == 0) {
                std::cout << "[Consumer] Retrying createXferReq (" << retryCount << ")...\n";
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
    } while (st != NIXL_SUCCESS && retryCount < 600);  // 60 seconds
    nixl_exit_on_failure(st, "Failed to create transfer request", CONSUMER_NAME);
    std::cout << "[Consumer] Transfer request created (retries: " << retryCount << ")\n";

    st = agent.postXferReq(xferReq, &xferParams);
    if (st != NIXL_SUCCESS && st != NIXL_IN_PROG) {
        nixl_exit_on_failure(st, "Failed to post transfer request", CONSUMER_NAME);
    }
    std::cout << "[Consumer] Transfer request posted\n";

    // 10. Wait for transfer completion
    std::cout << "[Consumer] Waiting for transfer completion...\n";
    auto startTime = std::chrono::steady_clock::now();
    while (true) {
        st = agent.getXferStatus(xferReq);
        if (st == NIXL_SUCCESS) {
            std::cout << "[Consumer] Transfer completed!\n";
            break;
        } else if (st != NIXL_IN_PROG) {
            nixl_exit_on_failure(st, "Transfer failed", CONSUMER_NAME);
        }

        auto elapsed = std::chrono::steady_clock::now() - startTime;
        if (std::chrono::duration_cast<std::chrono::seconds>(elapsed).count() > TIMEOUT_SEC) {
            std::cerr << "[Consumer] Timeout waiting for transfer completion\n";
            return 1;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
