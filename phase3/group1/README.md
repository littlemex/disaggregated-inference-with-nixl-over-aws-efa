...
#### 短期（次回セッション）- Disaggregated Inference 実行テスト

**現在の状態**: engine_id 問題解決、Producer/Consumer 再起動成功

**準備完了**:
- [OK] Producer: engine_id `phase3-nixl-efa-di-20260307` で起動
- [OK] Consumer: engine_id `phase3-nixl-efa-di-20260307` で起動
- [OK] NIXL 初期化成功（EFA devices, Rails 作成）
- [TODO] Proxy Server 再起動

**次のアクション**:

1. **Proxy Server の起動**
   - disagg_proxy_server.py を Node1 で再起動
   - Health check 確認

2. **Disaggregated Inference テスト実行**
   - Proxy Server 経由でテストリクエスト送信
   - NIXL handshake 成功を確認（engine_id mismatch エラーが解消）
   - Producer/Consumer 間の KV 転送を確認

3. **NIXL Two-Sided Messaging の動作確認**
   - postWrite/postRead の発生確認
   - fi_senddata/fi_recv の疎通確認
   - READ_REQUEST control message の送受信確認

4. **ログ分析**
   - kv_transfer_params の内容確認
   - remote_engine_id の受信確認
   - データ転送の成功を確認
...
#### vLLM v0.17.0 の engine_id 問題

**問題**: engine_id を指定しない場合、各インスタンスがランダム UUID を生成
- Producer: `495b4444-8935-4ada-a9d7-fe915b9c9595`
- Consumer: `9690d2d6-a66f-4fd8-905c-fd03dce5655f`
- 結果: NIXL handshake で engine_id mismatch エラー

**解決策**: kv-transfer-config に共通の engine_id を明示的に指定
```json
{
  "engine_id": "phase3-nixl-efa-di-20260307",
  ...
}
```
...
**誤解**: Consumer の kv_ip に Producer の IP を指定
**正解**: kv_ip には自ノードの IP を指定（ZMQ ソケットを bind するため）

- Producer: `"kv_ip": "172.31.2.221"` (自ノードの IP)
- Consumer: `"kv_ip": "172.31.10.117"` (自ノードの IP)

**remote_host/remote_port**: kv_transfer_params で Producer から Consumer に渡される
...
- 調査記録: `/home/coder/phase2/disaggregated-inference-with-nixl-over-aws-efa/phase3/group1/INVESTIGATION_SUMMARY_2026-03-07.md`

---
最終更新: 2026-03-07T20:05
